...
export interface ConstructionToolState {
  ...
  /** Numeric ratio for the scalar homothety tool. */
...
export class ConstructionToolController {
  ...
  activate(tool: ConstructionTool): ConstructionToolState {
    ...
    this.stateValue = {
      ...
      homothetyRatio: this.stateValue.homothetyRatio,
    };
    return this.state;
  }
  ...
  setHomothetyRatio(ratio: number): ConstructionToolState {
    if (!Number.isFinite(ratio)) return this.state;
    this.stateValue = { ...this.stateValue, homothetyRatio: ratio };
    return this.state;
  }
  ...
  handleObjectClick(objectId: string, document: GeometryDocument): ConstructionToolResult {
    ...
    if (this.stateValue.activeTool === "homothety_scalar" && step === 1 && this.stateValue.homothetyRatio === 0 && object.kind !== "point") {
      return this.fail("A zero homothety ratio is only supported for points.");
    }
...
function createConstruction(
  ...
  switch (tool) {
    ...
    case "homothety_scalar": {
      ...
      if (homothetyRatio === 0 && source.kind !== "point") {
        throw new Error("A zero homothety ratio is only supported for points");
      }
      const id = nextObjectId(document, "ht");
      const obj = {
        id,
        ...
        kind: source.kind,
        visible: true,
        definition: { type: "homothety_scalar", center: first, object: second, ratio: homothetyRatio },
      } as HomothetyScalar;
      return [obj];
    }
    case "inversion": {
